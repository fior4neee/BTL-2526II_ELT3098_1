...
class VietnamCoverageAnalyzer:
    ...
    def recommended_constellation(service: str = "internet") -> WalkerDelta:
        ...
        OPTIMIZED 2026-05-26: Mission-Split → Unified low-incl Walker.
        Single constellation @ h=1200km, i=22° serves BOTH Internet/VoIP (Ku band)
        AND Data/Weather (Ka band) on the same satellites.

        Rationale:
        - Low inclination (22°) maximizes dwell time over Vietnam latitude band
          (38% of orbit vs 11% at i=53°)
        - High altitude (1200km) gives larger footprint (radius 1714 km @ ε=25°
          vs 857 km @ h=500km) → covers all of Vietnam from one sat
        - Single constellation eliminates the 90 redundant Weather sats
        - Total: 48 sats vs 346 (86% reduction)
        - Validated 24h: 100% gateway coverage @ ε=20-25° (real gw thresholds)
        - Latency Internet @ h=1200: ~8ms one-way (well under 50ms VoIP target)

        service : "internet"/"voip"/"weather"/"unified" all return the SAME
                  unified constellation (different RF band, same satellites).
        """
        service = service.lower()
        if service in ("internet", "voip", "internet_voip",
                       "weather", "data", "data_weather",
                       "unified"):
            # 1200 km, i=22° — Walker Delta 48/6/2
            # Validated by simulation (24h SGP4):
            #   - Hanoi  @ε=25°: 100.00%, gap=0s
            #   - Danang @ε=20°: 100.00%, gap=0s
            #   - HCMC   @ε=25°: 100.00%, gap=0s
            #   - VN grid @ε=15°: 100.00%, avg 2.79 sats visible
            #   - VN grid @ε=25°: 100.00%, avg 1.74 sats visible
            return WalkerDelta(N=48, P=6, F=2, inclination_deg=22.0, altitude_km=1200.0)
        else:
            raise ValueError(
                f"Unknown service '{service}'. "
                f"Use 'internet', 'voip', 'weather', or 'unified'."
            )
...
def main() -> None:
    """Run coverage analysis: baseline analytical estimates + optimized simulation."""
    import sys
    ...
    # ── Section 1: Analytical estimates for old baseline configs ─────────────
    print("\n[1] Analytical Walker Delta Minimum Satellite Estimates")
    print("    (Baseline i=53° — comparison only; replaced by unified 48/6/2)")
    print("─" * 60)
    for label, h, inc in [
        ("Internet/VoIP  h=500 km,  i=53°", 500,  53.0),
        ("Data/Weather   h=1000 km, i=53°", 1000, 53.0),
    ]:
        est = estimate_minimum_satellites(h, el_min_deg=15.0, inclination_deg=inc)
        print(f"\n  {label}:")
        for k, v in est.items():
            print(f"    {k:<28} = {v}")

    # ── Section 2: Full simulation with optimized unified constellation ───────
    print("\n\n[2] Full Coverage Simulation — Optimized Unified Constellation")
    print("─" * 60)
    c = VietnamCoverageAnalyzer.recommended_constellation("unified")
    print(f"  Config: {c}")
    print(f"  T={c.N}, P={c.P}, F={c.F}, i={c.inclination_deg}°, h={c.altitude_km} km\n")
    ...
    analyzer.export_tle(out_dir / "constellation.tle")

    print("\n" + "=" * 72)
    status = "✓ PASS" if metrics.coverage_pct >= 99.9 else "✗ FAIL"
    print(f"  Overall coverage: {metrics.coverage_pct:.3f}%  [{status}]")
    print(f"  Max gap: {metrics.max_gap_s:.0f} s  |  "
          f"RTT: {metrics.round_trip_delay_ms:.1f} ms")
