...
func main() {
	...
	// Path is resolved relative to core_network/ working directory.
	tlePath := deployConfig.TlePath
	if tlePath == "" {
		// Fallback chain: try common locations in order
		candidates := []string{
			"../orbit_calc/outputs/unified/constellation.tle",
			"../orbit_calc/outputs/internet_voip/constellation.tle",
			"data/constellation.tle",
		}
		...
		if tlePath == "" {
			tlePath = candidates[0] // best guess; will fail later with clear msg
		}
	}
